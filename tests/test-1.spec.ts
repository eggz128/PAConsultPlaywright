...
test.beforeAll(async()=>{
  console.log("Runs before anything in this file")
})

test.beforeEach(async()=>{
  console.log("Runs before every test individually in this file")
})

test.afterAll(async()=>{
  console.log("Runs once all tests in this file complete")
})

test.afterEach(async()=>{
  console.log("Runs folowing each individual test")
})

// Annotate entire file as serial (opt out of parallel for this file onely).
test.describe.configure({ mode: 'serial' });

test.describe('A Suite', async () => {
  ...
  test('Login Test', async ({ page }) => { //"page" fixture is passed to the test. This takes are of openeing the browser and supplying a "page" to work with
    ...
    //await page.getByLabel('Password?').fill('edgewords123'); //AI suggestion for #passwordXXX fix
  ...
  test.describe('Another suite', async () => {
    test.setTimeout(30 * 1000);

    test.skip('Ecommerce', async ({ page }) => {
